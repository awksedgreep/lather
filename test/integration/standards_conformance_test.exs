defmodule Lather.Integration.StandardsConformanceTest do
    ...
    match "/soap" do
      Lather.Server.Plug.call(
        ...
        Lather.Server.Plug.init(
          service: Lather.Integration.StandardsConformanceTest.ConformanceService
        )
      )
    end
  end
  ...
  describe "SOAP 1.1 envelope structure" do
    ...
    test "response contains correct SOAP 1.1 namespace", %{base_url: base_url, port: port} do
      ...
      body =
        build_soap_11_request(
          "Echo",
          "<message>test</message>",
          "http://test.example.com/conformance"
        )
    ...
    test "response has soap:Envelope as root element", %{port: port} do
      ...
      assert String.contains?(response.body, "soap:Envelope") or
               String.contains?(response.body, "Envelope")

      assert String.contains?(response.body, "soap:Body") or
               String.contains?(response.body, "Body")
    end
  ...
  describe "WSDL structure conformance" do
    ...
    test "WSDL has required root element", %{port: port} do
      {:ok, response} =
        Finch.build(:get, "http://localhost:#{port}/soap?wsdl", [])
        |> Finch.request(Lather.Finch)

      assert response.status == 200

      assert String.contains?(response.body, "<definitions") or
               String.contains?(response.body, "definitions>")
    end
    ...
    test "WSDL contains types section or schema", %{port: port} do
      ...
      has_types =
        String.contains?(response.body, "<types") or String.contains?(response.body, "types>")

      has_schema =
        String.contains?(response.body, "schema") or String.contains?(response.body, "xsd:")

      has_messages =
        String.contains?(response.body, "<message") or String.contains?(response.body, "message>")
    ...
    test "WSDL contains message definitions", %{port: port} do
      ...
      assert String.contains?(response.body, "<message") or
               String.contains?(response.body, "message>")
    end
    ...
    test "WSDL contains portType", %{port: port} do
      ...
      assert String.contains?(response.body, "<portType") or
               String.contains?(response.body, "portType>")
    end
    ...
    test "WSDL contains binding", %{port: port} do
      ...
      assert String.contains?(response.body, "<binding") or
               String.contains?(response.body, "binding>")
    end
    ...
    test "WSDL contains service", %{port: port} do
      ...
      assert String.contains?(response.body, "<service") or
               String.contains?(response.body, "service>")
    end
  ...
  describe "SOAP fault structure" do
    ...
    test "client fault has correct structure", %{port: port} do
      ...
      assert String.contains?(response.body, "faultcode") or
               String.contains?(response.body, "Code")

      assert String.contains?(response.body, "faultstring") or
               String.contains?(response.body, "Reason")
    end
    ...
    test "server fault has correct faultcode", %{port: port} do
      ...
      assert String.contains?(response.body, "Server") or
               String.contains?(response.body, "Receiver")
    end
  ...
  describe "HTTP requirements" do
    ...
    test "returns 405 for unsupported HTTP methods", %{port: port} do
      {:ok, response} =
        Finch.build(:put, "http://localhost:#{port}/soap", [], "test")
        |> Finch.request(Lather.Finch)
  ...
  defp start_server(_context) do
    ...
    {:ok, server_pid, actual_port} = Lather.TestUtils.start_server(ConformanceRouter, port)
